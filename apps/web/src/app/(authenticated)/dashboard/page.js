...
import { CardSkeleton, EmptyState, DashboardGrid, registerWidget, getAllWidgets, JourneyLauncher, JourneyCard, useJourneys, } from '@ui';
...
registerWidget({
    ...
    component: () => (<div className="space-y-3">
      {[
        ...
        ].map((item, i) => (<div key={i} className="flex items-center justify-between py-1">
          <div>
...
registerWidget({
    id: 'quick-stats',
    title: 'Quick Stats',
    component: () => (<div className="grid grid-cols-2 gap-3">
      {[
        ...
        ].map((stat, i) => (<div key={i} className="p-3 rounded-lg bg-[var(--y-bg)] border border-[var(--y-border)]">
...
registerWidget({
    ...
    component: () => (<div className="space-y-2">
      <EmptyState title="No upcoming events" description="Create your first event in CheckIN" action={{ label: 'Create Event', onClick: () => { } }}/>
    </div>),
    defaultWidth: 'third',
});
export default function Dashboard() {
    ...
    const { state, journeys, activeJourney, activeProgress, startJourney, completeStep, skipStep, setCurrentStep, resetJourney, dismissWelcome, getAvailableJourneys, getCoachMessage, getAchievements, } = useJourneys();
    ...
    return (<div className="space-y-6">
      ...
      {state.showWelcome && (<JourneyLauncher journeys={journeys} completedJourneys={state.completedJourneys} onStartJourney={startJourney} onDismiss={dismissWelcome}/>)}

      {/* Active Journey Progress */}
      {activeJourney && activeProgress && (<div className="bg-[var(--y-surface-elevated)] rounded-xl p-6 border border-[var(--y-border)]">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <button className="text-sm text-[var(--y-text-secondary)] hover:text-[var(--y-text-primary)] transition-colors" onClick={() => resetJourney(activeJourney.id)}>
              Reset journey
            </button>
          </div>
          ...
          <div className="space-y-2">
            {activeJourney.steps.map((step) => {
                ...
                if (isCompleted) {
                    ...
                    statusIcon = '🔒';
                    statusClass = 'bg-gray-500/5 text-gray-300';
                }
                else {
                    statusIcon = '○';
                    statusClass = 'bg-gray-500/10 text-gray-400';
                }
                return (<div key={step.id} className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors
                    ${isActive ? 'ring-1 ring-[var(--y-brand-primary)]' : 'hover:bg-[var(--y-bg)]'}`} onClick={() => setCurrentStep(activeJourney.id, step.id)}>
                  ...
                  {!isCompleted && !isSkipped && !isLocked && (<div className="flex gap-1 shrink-0">
                      <button className="text-xs px-2 py-1 rounded bg-[var(--y-brand-primary)] text-white hover:opacity-90" onClick={(e) => { e.stopPropagation(); completeStep(activeJourney.id, step.id); }}>
                        Done
                      </button>
                      <button className="text-xs px-2 py-1 rounded text-[var(--y-text-secondary)] hover:bg-[var(--y-bg)]" onClick={(e) => { e.stopPropagation(); skipStep(activeJourney.id, step.id); }}>
                        Skip
                      </button>
                    </div>)}
                </div>);
          ...
          <div className="mt-4 h-1.5 bg-[var(--y-border)] rounded-full overflow-hidden">
            <div className="h-full bg-[var(--y-brand-primary)] rounded-full transition-all duration-700" style={{
      ...
      {/* Available Journeys */}
      {!activeJourney && availableJourneys.length > 0 && (<div>
          <h2 className="text-lg font-semibold text-[var(--y-text-primary)] mb-3">
            Get started with a journey
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {availableJourneys.map((journey) => (<JourneyCard key={journey.id} journey={journey} progress={state.journeyProgress[journey.id]} onStart={startJourney} onResume={startJourney} onReset={resetJourney}/>))}
          </div>
        </div>)}

      {/* AI Coach — contextual coaching on active journey */}
      {coachMessage && (<div className="bg-gradient-to-r from-blue-500/5 to-purple-500/5 rounded-xl p-5 border border-blue-500/20">
          <div className="flex items-start gap-4">
            ...
            <button className="shrink-0 text-sm px-4 py-2 rounded-lg bg-[var(--y-brand-primary)] text-white hover:opacity-90 transition-opacity" onClick={() => setCurrentStep(coachMessage.journeyId, coachMessage.stepId)}>
              Start next step
            </button>
          </div>
        </div>)}
      ...
      {achievements.length > 0 && (<div>
          ...
          <div className="flex gap-3 overflow-x-auto pb-2">
            {achievements.map((a) => (<div key={a.id} className="flex items-center gap-3 px-4 py-3 rounded-xl bg-[var(--y-surface-elevated)] border border-[var(--y-border)] shrink-0">
      ...
      <DashboardGrid widgets={widgets.map((w) => ({ id: w.id }))}/>
    </div>);
}
